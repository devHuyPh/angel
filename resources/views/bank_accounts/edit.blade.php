<form action="{{ route('bank_accounts.update', $account->id) }}" method="post">
     @csrf @method('PUT')
    <div class="modal fade" id="editBankAccount_{{$account->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">{{trans('core/base::layouts.edit_bank_account')}}</h5>
            ...
            <div class="modal-body">
                ...
                <div >
                    <div class="mb-3">
                        ...
                        <input type="text" name="bank_code_new" id="custom_bank_code" class="form-control" value="{{ $account->bank_code }}" placeholder="Ex: ACB">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">{{trans('core/base::layouts.bank_name')}}<span class="text-danger">*</span></label>
                        <input type="text" name="bank_name" id="bank_name" class="form-control" value="{{ $account->bank_name }}" placeholder="Ex: Ngân hàng Á Châu">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">{{trans('core/base::layouts.account_number')}}<span class="text-danger">*</span></label>
                    <input type="text" name="account_number" class="form-control" value="{{ $account->account_number }}"  placeholder="Ex: 09999999999">
                </div>
                <div class="mb-3">
                    <label class="form-label">{{trans('core/base::layouts.account_holder')}}<span class="text-danger">*</span></label>
                    <input type="text" name="account_holder" class="form-control" value="{{ $account->account_holder }}" placeholder="Ex: NGUYEN VAN A">
                </div>
                <div class="mb-3">
                    <label class="form-label">{{trans('core/base::layouts.branch')}} {{trans('core/base::layouts.none_required')}}</label>
                    <input type="text" name="branch" class="form-control" value="{{ $account->branch }}" placeholder="Ex: Nam Từ Liêm, Hà Nội">
                </div>
                <div class="mb-3">
                    <label class="form-label">{{trans('core/base::layouts.swift_code')}} {{trans('core/base::layouts.none_required')}}</label>
                    <input type="text" name="swift_code" class="form-control" value="{{ $account->swift_code }}" placeholder="Ex: 1234ADSEF">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{trans('core/base::layouts.close')}}</button>
                <button type="submit" class="btn btn-primary">{{trans('core/base::layouts.edit')}}</button>
            </div>
            </div>
        </div>
    </div>
