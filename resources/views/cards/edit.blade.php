  ...
  <div class="container-fluid">
    ...
    <div class="col-12">
      ...
      <div class="card-header bg-warning text-white">
        ...
        <i class="fas fa-edit me-2"></i>{{ __('core/base::layouts.edit_title') }}
        </h3>
      </div>
      <div class="card-body">
        <form action="{{ route('cards.edit.update', $card->id) }}" method="POST" enctype="multipart/form-data"
        id="cardForm">
        @csrf
        <div class="row">
          <!-- Card Name -->
          ...
          <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
            value="{{ old('name', $card->name) }}" placeholder="{{ __('core/base::layouts.name_placeholder') }}"
            required>
          @error('name')
      ...
      @enderror
          ...
          <input type="text" class="form-control @error('number') is-invalid @enderror" id="number" name="number"
            ...
            placeholder="{{ __('core/base::layouts.number_placeholder') }}" maxlength="20" required>
          @error('number')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
          </div>
          ...
          <div class="input-group">
            <input type="float" class="form-control @error('cashback') is-invalid @enderror" id="cashback"
            name="cashback" value="{{ old('cashback', $card->cashback) }}"
            placeholder="{{ __('core/base::layouts.cashback_placeholder') }}" step="0.01" min="0" max="100">
      ...
      @enderror
          ...
          <label for="value" class="form-label">
            <i class="fas fa-dollar-sign me-1"></i>{{ __('core/base::layouts.value_card') }} <span
            class="text-danger">*</span>
          </label>
          <div class="input-group">
            <span class="input-group-text">$</span>
            <input type="number" class="form-control @error('value') is-invalid @enderror" id="value" name="value"
            value="{{ old('value', $card->value) }}"
            placeholder="{{ __('core/base::layouts.value_placeholder') }}" step="0.01" min="0" required>
          </div>
          @error('value')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
          ...
          <label for="expiration_date" class="form-label">
            <i class="fas fa-calendar-alt me-1"></i>{{ __('core/base::layouts.expiration_date') }}
          </label>
          <input type="month" class="form-control @error('expiration_date') is-invalid @enderror"
            id="expiration_date" name="expiration_date"
            value="{{ old('expiration_date', \Carbon\Carbon::parse($card->expiration_date)->format('Y-m')) }}"
            min="{{ date('Y-m') }}">
      ...
      @enderror
          ...
          <!-- Gift Description -->
          <div class="col-md-6 mb-3">
          <label for="gift_description" class="form-label">
            <i class="fas fa-gift me-1"></i>{{ __('core/base::layouts.gift_description') }}
          </label>
          <textarea class="form-control @error('gift_description') is-invalid @enderror" id="gift_description"
            name="gift_description" rows="3"
            placeholder="{{ __('core/base::layouts.gift_description_placeholder') }}"
            maxlength="500">{{ old('gift_description', $card->gift_description) }}</textarea>
          <div class="form-text">
      ...
      @enderror
          ...
          <!-- Preview -->
          <div class="mt-3" id="imagePreviewContainer" style="{{ $card->image ? '' : 'display:none;' }}">
            ...
            <img id="imagePreview" src="{{ $card->image_url ?? '/placeholder.svg' }}"
              alt="{{ __('core/base::layouts.image_preview_alt') }}" class="img-fluid rounded shadow-sm"
              style="max-height: 250px; width: auto; display: block; margin: 0 auto;">
            <div class="mt-3 text-center">
              <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeImage()">
              <i class="fas fa-trash me-1"></i> {{ __('core/base::layouts.remove_image') }}
              </button>
            </div>
            </div>
          </div>
        ...
        <div class="row">
          ...
          <div class="d-flex justify-content-between align-items-center">
            ...
            <i class="fas fa-arrow-left me-1"></i> {{ __('core/base::layouts.back_to_list') }}
            </a>
            <div>
            ...
            <button type="submit" class="btn btn-warning" id="submitBtn">
              <i class="fas fa-save me-1"></i> {{ __('core/base::layouts.update_button') }}
            </button>
            </div>
          </div>
          </div>
...
@push('footer')
  <script>
    ...
    const giftTextarea = document.getElementById('gift_description');
    ...
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> {{ __('core/base::layouts.updating') }}';
    });
    ...
    document.getElementById('gift_description').addEventListener('input', updateGiftCounter);
