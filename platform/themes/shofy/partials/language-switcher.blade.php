                ...
                @else
                    <div class="d-flex align-items-center text-white gap-3">
                        @foreach ($supportedLocales as $localeCode => $properties)
                            @continue($localeCode === Language::getCurrentLocale())
                            <span>
                            <a href="{{ Language::getSwitcherUrl($localeCode, $properties['lang_code']) }}" class="d-flex gap-2 align-items-center justify-content-center">
                                @if ($languageDisplay === 'all' || $languageDisplay === 'flag')
                                    {!! language_flag($properties['lang_flag'], $properties['lang_name']) !!}
                                @endif
                                @if ($languageDisplay === 'all' || $languageDisplay === 'name')
        ...
        @else
            <div class="offcanvas__select language">
                <div class="offcanvas__lang d-flex align-items-center justify-content-md-end">
                    <div class="offcanvas__lang-img mt-2 ms-3">
                        @if ($languageDisplay === 'all' || $languageDisplay === 'flag')
                            {!! language_flag(Language::getCurrentLocaleFlag(), Language::getCurrentLocaleName(), 24) !!}
                        @endif
                    </div>
                    <div class="offcanvas__lang-wrapper">
                    <span class="offcanvas__lang-selected-lang tp-lang-toggle text-light" id="tp-offcanvas-lang-toggle">
                        @if ($languageDisplay === 'all' || $languageDisplay === 'name')
                            {{ Language::getCurrentLocaleName() }}
                        @endif
                    </span>
