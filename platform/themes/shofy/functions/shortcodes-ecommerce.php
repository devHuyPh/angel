...
app()->booted(function (): void {
    ...
    // Modern Categories Slider Shortcode
    Shortcode::register(
        'ecommerce-modern-categories',
        __('Modern Categories Slider'),
        __('Display categories in a modern slider with glassmorphism design'),
        function (ShortcodeCompiler $shortcode) {
            $categoryIds = Shortcode::fields()->getIds('category_ids', $shortcode);
            ...
            $shortcode->background_color = $shortcode->background_color ?: 'transparent';
            $shortcode->show_products_count = $shortcode->show_products_count !== 'no';
            $shortcode->items_per_view = (int) ($shortcode->items_per_view ?: 4);

            return Theme::partial('shortcodes.ecommerce-modern-categories.index', compact('shortcode', 'categories'));
        }
    );

    Shortcode::setPreviewImage('ecommerce-modern-categories', Theme::asset()->url('images/shortcodes/ecommerce-categories/slider.png'));

    Shortcode::setAdminConfig('ecommerce-modern-categories', function (array $attributes) {
        return ShortcodeForm::createFromArray($attributes)
            ->withLazyLoading()
            ->add(
                'category_ids',
