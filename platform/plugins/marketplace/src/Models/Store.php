...
class Store extends BaseModel
{
    ...
    public function scopeLowerLevelInArea($query, Store $store)
    {

        // dd($store);
        if ($store->store_level_id == null) {
            return $query;
        } else {
            $query->where('store_level_id', '<', $store->store_level_id);

            if ($store->store_level_id == 3) {
                $query->where('state', $store->state);
            } elseif ($store->store_level_id == 2) {
    ...
    public function myStoreOrders(): HasMany
    {
        return $this
            ->hasMany(StoreOrder::class, 'to_store');
    }

    public function fromMyStoreOrders(): HasMany
    {
        return $this
            ->hasMany(StoreOrder::class, 'from_store');
    }

    public function lateDeliveries(): HasMany
    {
        return $this->hasMany(VendorLateDelivery::class, 'store_id');
    }
}
