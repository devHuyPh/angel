...
class OrderSupportServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        $this->app->booted(function (): void {
            ...
            // Cho phép cấu hình số dư tối thiểu để dùng ví (wallet_min_amount)
            add_filter('ecommerce_payment_settings_additional_fields', function ($fields) {
                $fields['wallet_min_amount'] = [
                    'type' => 'number',
                    'label' => __('Số dư ví tối thiểu để sử dụng ví'),
                    'attributes' => [
                        'min' => 0,
                        'step' => '0.01',
                        'value' => setting('wallet_min_amount', 0),
                        'placeholder' => __('Nhập số tiền (VD: 100000)'),
                    ],
                ];
    ...
    public function processPostCheckoutOrder(
        ...
        // $storeIdFromRequest = ;
        // dd($storeIdFromRequest);
        $sessionCheckoutData[] = $request->input('store_id');
        $orderss = OrderHelper::setOrderSessionData($token, $sessionCheckoutData);
        //    dd($orderss);
        if ($couponCode) {
            ...
            foreach ($mpSessionData as &$storeCheckoutData) {
                ...
                Arr::set($storeCheckoutData, 'store_id', $request->input('store_id', null));
            }
        ...
        $mpSessionData = Arr::get($sessionCheckoutData, 'marketplace', []);
        foreach ($mpSessionData as &$storeCheckoutData) {
            Arr::set($storeCheckoutData, 'store_id', $request->input('store_id'));
        }
        $sessionCheckoutData = OrderHelper::setOrderSessionData($token, [
            'marketplace' => $mpSessionData,
        ]);
        // dd($sessionCheckoutData);
        ...
        foreach ($groupedProducts as $storeId => $productsInStore) {
            ...
            // if ($storeId != $storeIdFromRequest) {
            //     \Log::warning('Mismatched store ID:', ['request' => $storeIdFromRequest, 'group' => $storeId]);
            //     continue;
            // }
            ...
            // dd($token);
            $store_id= $request->input('store_id', $storeId);
            // if($token){
            //     OrderHelper::updateStore($token, $store_id, $order);
            // } else {
            //     $token = OrderHelper::getOrderSessionToken();
            // }
            $orders[$storeId] = $this->handleCheckoutOrderByStore(
                ...
                $currentUserId,
                $order,
                $sessionStoreData['store_id'] ?? $storeId,
                $discounts,
                $promotionService,
                $shippingFeeService,
                $applyCouponService
        ...
        Log::info('Marketplace payment debug: after apply_filters', [
        ...
        if ($paymentData['error'] || ! $paymentData['charge_id']) {
            ...
            Log::warning('Marketplace checkout payment redirect with error', [
                'payment_method' => $request->input('payment_method'),
    ...
    public function processPaymentMethodPostCheckout(Request $request, int|float $totalAmount): array
    {
        ...
        if ($request->input('payment_method') === $walletMethod) {
            Log::info('Marketplace wallet payment start', [
            ...
            if (! auth('customer')->check()) {
                return [
                    'error' => true,
                    'message' => __('Vui lòng đăng nhập để thanh toán bằng ví.'),
                ];
            }
            ...
            if (! $customer) {
                return [
                    'error' => true,
                    'message' => __('Không tìm thấy thông tin khách hàng.'),
                ];
            }
            ...
            $minWallet = (float) setting('wallet_min_amount', 0);
            ...
            if ($orders->isEmpty()) {
                return [
                    'error' => true,
                    'message' => __('Không tìm thấy đơn hàng để thanh toán.'),
                ];
            }
            ...
            if ($orderTotal <= 0) {
                return [
                    'error' => true,
                    'message' => __('Tổng đơn hàng không hợp lệ.'),
                ];
            }
            ...
            try {
                DB::transaction(function () use (
                    ...
                    if ($minWallet > 0 && $currentWallet < $minWallet) {
                        $message = __('Cần tối thiểu :amount trong ví để dùng ví thanh toán. Vui lòng chọn chuyển khoản.', [
                            'amount' => format_price($minWallet),
                        ]);
                    ...
                    foreach ($orders as $order) {
                        ...
                        $remaining = max((float) $order->amount - $useForOrder, 0);
                    ...
                    // Với partial, chúng ta sẽ tạo payment SePay riêng cho phần còn lại, nên chỉ lưu log ví ở order history.
                    if (! $isPartial) {
                ...
                Log::error('Marketplace wallet payment error', [
            ...
            // Nếu còn thiếu, chuyển qua SePay cho phần còn lại
            if ($remainingAfterWallet > 0) {
                $sepayEnabled = is_plugin_active('fob-sepay') && (int) get_payment_setting('status', SEPAY_PAYMENT_METHOD_NAME, 0) === 1;

                if (! $sepayEnabled) {
                    return [
                        'error' => true,
                        'message' => __('Vui lòng chọn chuyển khoản ngân hàng để thanh toán phần còn lại.'),
                    ];
                }

                $sepayPrefix = get_payment_setting('prefix', SEPAY_PAYMENT_METHOD_NAME, 'SDH');
                $sepayChargeId = $sepayPrefix . sprintf('%\'.09d', (int) (microtime(true) * 10));

                $sepayMetadata = array_merge($metadata, [
                    'wallet_remaining' => $remainingAfterWallet,
                ]);

                do_action(PAYMENT_ACTION_PAYMENT_PROCESSED, [
                    'amount' => $remainingAfterWallet,
                    'currency' => $request->input('currency'),
                    'charge_id' => $sepayChargeId,
                    'order_id' => $orderIds,
                    'customer_id' => $customer->getKey(),
                    'customer_type' => Customer::class,
                    'payment_channel' => SEPAY_PAYMENT_METHOD_NAME,
                    'status' => PaymentStatusEnum::PENDING,
                    'metadata' => $sepayMetadata,
                    'wallet_payment' => [
                        'allocations' => $allocations,
                        'wallet_used' => $walletUsed,
                        'remaining' => $remainingAfterWallet,
                ...
                return [
                    'error' => false,
                    'message' => __('Ví đã trừ :wallet, chuyển sang SePay để thanh toán thêm :remaining.', [
                    ...
                    'type' => SEPAY_PAYMENT_METHOD_NAME,
                    'charge_id' => $sepayChargeId,
                    'metadata' => $sepayMetadata,
                    'wallet_payment' => [
                        'wallet_used' => $walletUsed,
            ...
            // Thanh toán hết bằng ví
            return [
                'error' => false,
                'message' => __('Đã thanh toán toàn bộ bằng ví.'),
        ...
        Log::info('Marketplace payment debug: before apply_filters', [
            'payment_method' => $request->input('payment_method'),
            'amount' => $paymentData['amount'],
            'order_id' => $request->input('order_id'),
            'wallet_password_filled' => $request->filled('wallet_password'),
        ]);
