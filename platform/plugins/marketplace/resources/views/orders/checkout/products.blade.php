...
<div class="checkout-products-marketplace shipping-method-wrapper">
    {{-- new bill duong --}}
    <div id="store-new"></div>
    <div id="list-pro-new"></div>
    <div id="shipping-method-new"></div>
    {{-- end new bill duong --}}
    @foreach ($groupedProducts as $grouped)
        @php
            ...
            if (!$store->exists) {
                $store->id = 0;
                $store->name =
                    get_ecommerce_setting('company_name_for_invoicing', get_ecommerce_setting('store_name')) ?:
                    \Botble\Theme\Facades\Theme::getSiteTitle();
            ...
            $isAvailableShipping =
                Arr::get($sessionData, 'is_available_shipping', true) &&
                !(bool) get_ecommerce_setting('disable_shipping_options', false);

            $orderAmount = max($rawTotal - $promotionDiscountAmount - $couponDiscountAmount, 0);
            $orderAmount += (float) $shippingAmount;
        @endphp
        <div class="mt-3 bg-light mb-3" style="display: none">
            <div class="p-2" style="background: antiquewhite;">
                <img class="img-fluid rounded"
                    ...
                    alt="{{ $store->name }}" style="max-width: 30px; margin-inline-end: 3px;">
                <span class="font-weight-bold" data-storeid="{{ $store->id }}">{!! BaseHelper::clean($store->name) !!}</span>
                ...
                <div class="shipping-method-wrapper py-3">
                    @if (!empty($shipping))
                        <div class="payment-checkout-form">
                            ...
                            <input name="shipping_option[{{ $storeId }}]" type="hidden"
                                        ...
                                        @foreach ($shippingItems as $shippingOption => $shippingItem)
                                            @include('plugins/ecommerce::orders.partials.shipping-option', [
                                                ...
                                                'attributes' => [
                                                    ...
                                                    'checked' =>
                                                        old("shipping_method.$storeId", $defaultShippingMethod) ==
                                                            $shippingKey &&
                                                        old("shipping_option.$storeId", $defaultShippingOption) ==
                                                            $shippingOption,
