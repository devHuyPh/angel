...
@section('content')
    @php
        $aEventNone = '';
        $disabled = '';
        if ($order->no_change == 1) {
            $aEventNone = ' style="pointer-events: none !important;" ';
            $disabled = 'disabled';
        }
        // {{$aEventNone}} {{$disabled}}
    @endphp
    <div id="main-order-content">
        ...
        <div class="row row-cards">
            <div class="col-md-8">
                            ...
                            <div class="col-md-6 offset-md-6">
                                ...
                                @if ($order->isInvoiceAvailable())
                                    <div class="text-end my-3">
                                        <x-core::button tag="a" :href="route('marketplace.vendor.orders.generate-invoice', $order->id)" target="_blank" icon="ti ti-download">
                                            {{ trans('plugins/ecommerce::order.download_invoice') }}
                                        </x-core::button>
                                    </div>
                                @endif

                                @include('plugins/ecommerce::orders.edit.form-edit', [
                                    'route' => 'marketplace.vendor.orders.edit',
                                ])
                            </div>
                        </div>
                    </x-core::card.body>
                        ...
                        @if ($order->status == Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                            <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-start gap-1">
                                    ...
                                    <div>
                                        <span
                                            class="text-uppercase">{{ trans('plugins/ecommerce::order.order_was_canceled') }}</span>
                                ...
                                <li @class(['step-item', 'user-action' => $history->user_id])>
                                    <div class="h4 m-0">
                                        @if (in_array($history->action, ['confirm_payment', 'refund']))
                                            <a {{$aEventNone}} class="show-timeline-dropdown text-primary"
                                                data-target="#history-line-{{ $history->id }}" href="javascript:void(0)">
                                            ...
                                            <x-core::table :striped="false" :hover="false" class="w-100">
                                                <x-core::table.body>
                                                    <x-core::table.body.row>
                                                        ...
                                                        <x-core::table.body.cell>
                                                            <a {{$aEventNone}} href="{{ route('marketplace.vendor.orders.edit', $order->id) }}"
                                                                title="{{ $order->code }}">
                                                                {{ $order->code }}
                                                            </a>
                                                        </x-core::table.body.cell>
                ...
                @include(
                    'plugins/ecommerce::orders.partials.digital-product-downloads-info',
                    compact('order'))
            </div>
                ...
                <x-core::card>
                    ...
                    <x-core::card.body class="p-0">
                        <div class="p-3">
                            ...
                            @if ($userInfo->email)
                                <p class="mb-1">
                                    <a {{$aEventNone}} href="mailto:{{ $userInfo->email }}">
                                        {{ $userInfo->email }}
                                    </a>
                                </p>
                            @endif
                        ...
                        @if (
                            !EcommerceHelper::countDigitalProducts($order->products) &&
                            ...
                            <div class="p-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    @if ($order->status != Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                                        <a {{$aEventNone}} class="btn-trigger-update-shipping-address btn-action text-decoration-none"
                    ...
                    @if ($order->canBeCanceledByAdmin())
                        <x-core::card.footer>
                            <x-core::button type="button" class="btn-trigger-cancel-order" :data-target="route('marketplace.vendor.orders.cancel', $order->id)">
...
@if (!EcommerceHelper::isDisabledPhysicalProduct() && $order->shipment && $order->shipment->id)
    <x-core::modal id="update-shipping-status-modal" :title="trans('plugins/ecommerce::shipping.update_shipping_status')" button-id="confirm-update-shipping-status-button"
