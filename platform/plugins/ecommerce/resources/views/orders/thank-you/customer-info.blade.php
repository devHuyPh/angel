...
<div class="order-customer-info">
    ...
    @if (is_plugin_active('payment') && $order->payment->id)
        ...
        @php
            $walletMeta = $order->payment->metadata ?? [];
            $walletUsed = (float) ($walletMeta['wallet_used'] ?? 0);
            $walletRemaining = (float) ($walletMeta['wallet_remaining'] ?? ($walletMeta['remaining_amount'] ?? 0));
        @endphp

        @if (! empty($walletMeta['wallet_payment']))
            <p>
                <span class="d-inline-block">{{ __('Wallet paid') }}:</span>
                <span class="order-customer-info-meta">{{ format_price($walletUsed) }}</span>
            </p>

            @if ($walletRemaining > 0)
                <p class="text-danger mb-1">
                    {{ __('Bạn cần chuyển khoản thêm :amount để hoàn tất đơn hàng.', ['amount' => format_price($walletRemaining)]) }}
                </p>
            @else
                <p class="mb-1">
                    {{ __('Đơn hàng đã được thanh toán toàn bộ bằng ví 1.') }}
                </p>
            @endif
        @endif
