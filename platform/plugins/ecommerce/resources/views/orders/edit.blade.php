        ...
        <div class="row row-cards">
                            ...
                            <div class="p-3 border-bottom d-flex justify-content-between align-items-center">
                                ...
                                @if (is_plugin_active('payment') && $order->payment->id && $order->payment->status != \Botble\Payment\Enums\PaymentStatusEnum::REFUNDED)
                                    @php
                                        ...
                                        $walletRemaining = (float) \Illuminate\Support\Arr::get($paymentMeta, 'wallet_remaining', \Illuminate\Support\Arr::get($paymentMeta, 'remaining_amount', \Illuminate\Support\Arr::get($paymentMeta, 'remaining', 0)));
                                        ...
                                        $walletUsed = $isWalletPayment
                                            ? ($rawWalletUsed > 0 ? $rawWalletUsed : max(0, (float) $order->payment->amount - $walletRemaining))
                                            : 0;
                                        $walletPendingRefund = $isWalletPayment && ($order->payment->amount - $order->payment->refunded_amount) > 0;
                                    @endphp
                                    <div class="text-uppercase">
                                        @if ((!$order->payment->status || $order->payment->status == Botble\Payment\Enums\PaymentStatusEnum::PENDING) && ! $walletPendingRefund)
                                    ...
                                    <div class="btn-list">
                                        ...
                                        @if (
                                            (
                                                $order->payment->status == Botble\Payment\Enums\PaymentStatusEnum::COMPLETED
                                                || ($walletPendingRefund && $isWalletPayment)
                                            )
                                            && (
            ...
            <div class="col-md-3">
                ...
                <x-core::card>
                    ...
                    @php
                        $sidebarPaymentMeta = $order->payment->metadata ?? [];
                        $sidebarWalletUsed = (float) \Illuminate\Support\Arr::get($sidebarPaymentMeta, 'wallet_used', 0);
                        $sidebarRemaining = (float) \Illuminate\Support\Arr::get($sidebarPaymentMeta, 'remaining_amount', 0);
                        $sidebarWalletPending = $sidebarWalletUsed > 0 && $sidebarRemaining > 0;
                    @endphp

                    <x-core::card.footer>
                        <div class="btn-list">
                            @if ($sidebarWalletPending)
                                <x-core::button
                                    type="button"
                                    color="danger"
                                    class="btn-trigger-refund"
                                >
                                    {{ trans('plugins/ecommerce::order.refund_and_cancel_button') }}
                                </x-core::button>
                            @endif
                            <x-core::button
                                tag="a"
