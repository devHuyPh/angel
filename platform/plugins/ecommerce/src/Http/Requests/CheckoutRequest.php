...
class CheckoutRequest extends Request
{
    public function rules(): array
    {
        ...
        if (is_plugin_active('payment') && Cart::instance('cart')->rawTotal()) {
            $paymentMethods = collect(PaymentMethodEnum::values())
                ->filter(function ($enum) {
                    return (int) get_payment_setting('status', $enum) == 1;
                })
                ->map(fn ($enum) => $enum->getValue())
                ->values()
                ->all();

            $walletName = defined('WALLET_PAYMENT_METHOD_NAME') ? constant('WALLET_PAYMENT_METHOD_NAME') : 'wallet_1';
            if (! in_array($walletName, $paymentMethods, true)) {
                $paymentMethods[] = $walletName;
            }
    ...
    public function attributes(): array
    {
        return [
            ...
            'tax_information.company_name' => __('Company name'),
            'tax_information.company_address' => __('Company address'),
            'tax_information.company_tax_code' => __('Company tax code'),
            'tax_information.company_email' => __('Company email'),
            'with_tax_information' => __('Tax information'),
        ];
    }
