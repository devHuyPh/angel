...
class OrderController extends BaseController
{
    ...
    public function postRefund(Order $order, RefundRequest $request)
    {
        ...
        $walletRemaining = (float) Arr::get($metadata, 'wallet_remaining', Arr::get($metadata, 'remaining_amount', Arr::get($metadata, 'remaining', 0)));
        $rawWalletUsed = (float) Arr::get($metadata, 'wallet_used', 0);
        $isWalletPayment = Arr::get($metadata, 'wallet_payment', false)
            || $walletRemaining > 0
            || $rawWalletUsed > 0;
        ...
        $walletRefundedAmount = (float) Arr::get($metadata, 'wallet_refunded_amount', 0);
        $walletRefundable = $isWalletPayment ? max(0, $walletUsed - $walletRefundedAmount) : 0;
        $availableGatewayRefund = max(0, $payment->amount - $payment->refunded_amount);
        $maxRefundable = $availableGatewayRefund + $walletRefundable;

        if (is_plugin_active('payment') && $request->input('refund_amount') > $maxRefundable) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage(
                    trans('plugins/ecommerce::order.refund_amount_invalid', [
                        'price' => format_price($maxRefundable, get_application_currency()),
                    ])
                );
        }
        ...
        if ($refundAmount > 0 && ($isWalletPayment ?? false)) {
            $walletRemaining = (float) Arr::get($metadata, 'wallet_remaining', Arr::get($metadata, 'remaining_amount', Arr::get($metadata, 'remaining', 0)));
            $walletUsed = $rawWalletUsed > 0 ? $rawWalletUsed : max(0, (float) $order->amount - $walletRemaining);

            $walletPreviouslyRefunded = (float) Arr::get($metadata, 'wallet_refunded_amount', 0);
            $walletRemainingToRefund = max(0, $walletUsed - $walletPreviouslyRefunded);
            $walletCreditAmount = min($refundAmount, $walletRemainingToRefund);

            if ($walletCreditAmount > 0 && $order->user instanceof Customer) {
                $customer = $order->user;
                $customer->walet_1 = (float) $customer->walet_1 + $walletCreditAmount;
                $customer->save();

                CustomerNotification::query()->create([
                    'title' => 'wallet_refund',
                    'dessription' => 'Hoàn ví 1: :amount cho đơn :code',
                    'variables' => json_encode([
                        'amount' => format_price($walletCreditAmount),
                        'code' => $order->code,
                    ]),
            ...
            if ($walletCreditAmount > 0) {
                $metadata['wallet_refunded_amount'] = $walletPreviouslyRefunded + $walletCreditAmount;
                $metadata['wallet_payment'] = Arr::get($metadata, 'wallet_payment', true);
                $metadata['wallet_used'] = $walletUsed;
                $payment->metadata = $metadata;
            }
        }
        ...
        if ($shouldCancelOrder && $order->status !== OrderStatusEnum::CANCELED) {
            $order = OrderHelper::cancelOrder(
                $order,
                trans('plugins/ecommerce::order.refund_and_cancel_order_reason'),
                trans('plugins/ecommerce::order.refund_and_cancel_order_description', [
