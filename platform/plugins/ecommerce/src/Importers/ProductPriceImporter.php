...
class ProductPriceImporter extends Importer
{
    ...
    public function columns(): array
    {
        return [
            ...
            ImportColumn::make('warehouse_price')
                ...
                ->rules(
                    ['nullable', 'numeric'],
                    trans('plugins/ecommerce::product-prices.import.rules.warehouse_price')
                ),
    ...
    public function examples(): array
    {
        $products = Product::query()
            ->take(5)
            ->wherePublished()
            ->select(['id', 'name', 'sku', 'cost_per_item', 'warehouse_price', 'price', 'sale_price'])
            ->get();
    ...
    public function handle(array $data): int
    {
        return Product::withoutTimestamps(function () use ($data) {
            return Product::query()->upsert(
                $data,
                ['id'],
                ['cost_per_item', 'warehouse_price', 'price', 'sale_price']
            ) / 2;
        });
    }
}
