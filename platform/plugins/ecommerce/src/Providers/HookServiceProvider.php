...
use Botble\Ecommerce\Supports\OrderHelper as OrderHelperSupport;
...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        // Đăng ký badge cho menu ngay từ đầu (không phụ thuộc RouteMatched)
        ...
        $this->app['events']->listen(RenderingDashboardWidgets::class, function (): void {
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = PendingLog::query()->where('status', 'pending')->count();

                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('plugins.ecommerce')
                    ->setTitle(trans('core/base::layouts.kyc'))
                    ->setKey('widget_total_kyc')
                    ->setIcon('ti ti-messages')
                    ->setColor('#074f9d')
                    ->setStatsTotal($items)
                    ->setRoute(route('dashboard.index'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 6, 2);
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                ...
                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('plugins.ecommerce')
                    ->setTitle(trans('core/base::layouts.kyc_pending'))
                    ->setKey('widget_total_kyc')
                    ->setIcon('ti ti-messages')
                    ->setColor('#074f9d')
                    ->setStatsTotal($items)
                    ->setRoute(route('kyc.pending'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 7, 2);

            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = VendorLateDelivery::on('mysql')
                    ->where('status', 0)
                    ->count();

                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('packages.mlm.vendor-late-delivery.index')
                    ->setTitle(trans('core/base::kho.late_delivery_menu_label'))
                    ->setKey('widget_total_late_delivery')
                    ->setIcon('ti ti-clock-hour-4')
                    ->setColor('#f59e0b')
                    ->setStatsTotal($items)
                    ->setRoute(route('vendor-late-delivery.index'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 8, 2);

            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = CustomerWithdrawal::query()
                    ->where('status', 'pending')
                    ->count();

                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('packages.mlm.marketing_withdrawals.view')
                    ->setTitle(trans('core/base::layouts.withdrawals_manager'))
                    ->setKey('widget_total_marketing_withdrawals')
                    ->setIcon('ti ti-wallet')
                    ->setColor('#16a34a')
                    ->setStatsTotal($items)
                    ->setRoute(route('withdrawals-manager.index'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 9, 2);
        });
        ...
        /**
         * ✅ XỬ LÝ KHI PAYMENT PROCESSED (ONLINE GATEWAY)
         * - Lưu local payment
         * - processOrder
         * - Chia lợi nhuận
         * - Cập nhật rank customer
         */
        if (defined('PAYMENT_ACTION_PAYMENT_PROCESSED')) {
            add_action(PAYMENT_ACTION_PAYMENT_PROCESSED, function (array $data): void {
                ...
                $walletPayment = Arr::get($data, 'wallet_payment');

                foreach ($orders as $order) {
                    $paymentData = $data;
                    $allocation = null;

                    if ($walletPayment && is_array($walletPayment)) {
                        $allocation = Arr::get($walletPayment, 'allocations.' . $order->id);
                    }

                    $paymentAmount = $allocation
                        ? max(0, Arr::get($allocation, 'remaining', $order->amount))
                        : $order->amount;

                    if ($allocation && $paymentAmount == 0 && Arr::get($allocation, 'wallet_used', 0) > 0) {
                        $paymentAmount = Arr::get($allocation, 'wallet_used', $order->amount);
                    }
                    ...
                    if ($allocation) {
                        $metadata = array_merge(
                            Arr::get($paymentData, 'metadata', []),
                            [
                                'wallet_payment' => true,
                                'wallet_used' => Arr::get($allocation, 'wallet_used', 0),
                                'wallet_remaining' => Arr::get($allocation, 'remaining', 0),
                            ]
                        );
                    ...
                    // Tạo/lưu payment local, tuỳ gateway sẽ set status COMPLETED/PENDING
                    PaymentHelper::storeLocalPayment($paymentData);
                }

                // Xử lý order như core (đặt is_finished, gửi mail, giảm stock...)
                app(OrderHelperSupport::class)->processOrder($orders->pluck('id')->all(), $data['charge_id']);

                /** @var ProfitSharingService $profitSharing */
                $profitSharing = app(ProfitSharingService::class);

                // Lấy lại danh sách payments theo charge_id
                ...
                foreach ($orders as $order) {
                    $order->refresh(); // đảm bảo relation/payment/status mới nhất

                    $payment = $payments->get($order->id) ?: $order->payment;

                    // Chỉ xử lý khi payment đã COMPLETED
                    if ($payment && $payment->status == PaymentStatusEnum::COMPLETED) {
                        // 1. Chia lợi nhuận
                        $profitSharing->shareProfitForOrder($order);

                        // 2. Cập nhật rank customer
                        if ($order->user instanceof Customer) {
        ...
        add_filter(FILTER_ECOMMERCE_PROCESS_PAYMENT, function (array $data, Request $request) {
            ...
            \Log::info('Ecommerce process payment: start', [
        ...
        /**
         * ✅ XỬ LÝ KHI ADMIN UPDATE PAYMENT (COD / BANK_TRANSFER / MANUAL)
         * - Khi status mới = COMPLETED:
         *   + confirmPayment (nếu là digital)
         *   + shareProfitForOrder
         *   + updateRank cho customer
         */
        if (defined('ACTION_AFTER_UPDATE_PAYMENT')) {
            add_action(ACTION_AFTER_UPDATE_PAYMENT, function ($request, $payment): void {
                /** @var Payment $payment */

                // Chỉ xử lý khi status mới là COMPLETED
                ...
                // Giữ logic confirmPayment cũ cho digital products
                if (
                ...
                // Nên có check anti-duplicate bên trong shareProfitForOrder
                $profitSharing->shareProfitForOrder($order);
    ...
    public function getPendingOrders(int|string|null $number, string $menuId): string
    {
        switch ($menuId) {
            ...
            case 'cms-core-kyc':
                if (! Auth::user()->hasPermission('plugins.ecommerce')) {
                    return $number;
                }

                $pendingCount = PendingLog::query()->where('status', 'pending')->count();
            ...
            case 'cms-core-nhamay':
                if (! Auth::user()->hasPermission('packages.mlm.factory-orders')) {
                    return $number;
                }

                $factoryOrders = StoreOrder::query()
                    ->whereNull('from_store')
                    ->where('status', 'pending')
                    ->count();

                if ($factoryOrders > 0) {
                    return BaseHelper::renderBadge(number_format($factoryOrders));
                }
    ...
    public function getMenuItemCount(array $data = []): array
    {
        ...
        if (Auth::check() && Auth::user()->hasPermission('packages.mlm.factory-orders')) {
            $factoryOrders = StoreOrder::query()
                ->whereNull('from_store')
                ->where('status', 'pending')
                ->count();
