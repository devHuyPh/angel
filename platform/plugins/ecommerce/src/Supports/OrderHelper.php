...
class OrderHelper
{
    ...
    public function setOrderCompleted(int|string $orderId, Request $request, int|string $userId = 0): Order
    {
        ...
        if ($order->payment && $order->payment->status == PaymentStatusEnum::COMPLETED) {
            ...
            app(ProfitSharingService::class)->shareProfitForOrder($order);
        }
        ...
        if ($store) {
            ...
            VendorNotifications::create([
                ...
                'description' => 'delivery_status_updated',
    ...
    // public function updateStore($token, $store_id, Order $order): void
    // {
    ...
    public function confirmPayment(Order $order): bool
    {
        ...
        $orderAmount = (float) $shipment->order->amount;
